...
'''
    ...
    emailDict['fromEmail'] = 'chris@yakjive.com'
    ...
    if (GLOBAL_DEBUG):
        ...
        for item in os.environ.keys():
            body += '<br>' + item + ':' + os.environ[item]

    try:
        sendEmail(emailDict['fromEmail'], emailDict['toEmail'], emailBody)
    except:
        # don't keep throwing exceptions
        log('Unable to send EXCEPTION email from ' + emailDict['fromEmail'] + ' to ' + emailDict['toEmail'])
        log('\t' + emailBody)

    replaceDict = {'hideStart':'<!--', 'hideEnd':'-->', 'userWelcome':''}
